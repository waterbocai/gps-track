...
def runSeatsByImeiTask(task_id,create_at,imei,fromTime,toTime):
    csvfile = getFileName(imei,create_at)+"_"+fromTime.split(" ")[0]+"_"+create_at.strftime("%H%M%S")
    seats=busdb.getSeatsByImei4BKTask(imei,fromTime,toTime,csvfile,ftp_addr)
    icfg.db.update("BackgroudTaskSequence",where='id=$task_id',vars=locals(),
                        state="finish",finished_at=datetime.datetime.now())
    return 
    
tasks = icfg.db.query("SELECT * FROM BackgroudTaskSequence WHERE state='waiting'")

threads = []
for task in tasks:
    icfg.db.update("BackgroudTaskSequence",where='id=$task.id',vars=locals(),
                         state="working",start_at=datetime.datetime.now())
